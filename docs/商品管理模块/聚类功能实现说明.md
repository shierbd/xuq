...
#### API 端点 (backend/routers/products.py)

**核心端点**:
- POST /api/products/cluster：执行聚类（同步），完成后自动落库簇汇总
- POST /api/products/cluster-large：大规模聚类（同步），完成后自动落库簇汇总
- POST /api/products/cluster-large/async：大规模聚类（异步任务）
- GET /api/tasks/{task_id}：查询异步任务进度/状态
- POST /api/products/cluster-keywords：生成簇关键词并落库
- POST /api/products/cluster-keywords/async：异步生成簇关键词并落库
- GET /api/products/cluster-keywords/{cluster_id}：查询单簇关键词
- GET /api/products/cluster/summary：即时生成簇汇总（不落库）
- POST /api/products/cluster/summary/persist：将簇汇总落库
- GET /api/products/cluster/summary/stored：读取已落库的簇汇总（包含 top_keywords）

**执行流程（当前实现）**:
1. 执行聚类（同步/异步）并更新 products.cluster_id / cluster_type
2. （可选）生成簇关键词并写入 product_cluster_keywords
3. 自动写入 product_cluster_summaries（聚类完成、关键词生成后都会触发）
4. 前端优先读取 /cluster/summary/stored，为空时回退 /cluster/summary

**返回增强**:
- 聚类完成和关键词生成接口会附带 summary_persisted 字段，表示簇汇总落库结果。
### 2. 前端实现
...
#### 2. 簇关键词表 (product_cluster_keywords)
...
#### 3. 簇汇总表 (product_cluster_summaries)

**插入数据**:
```json
{
  "cluster_id": 1,
  "cluster_name": "Excel Template",
  "cluster_name_cn": "Excel 模板",
  "cluster_size": 150,
  "avg_rating": 4.8,
  "avg_price": 9.99,
  "total_reviews": 187575,
  "example_products": "excel budget template; monthly expense tracker; ...",
  "top_keywords": "excel, template, budget, expense, tracker",
  "created_time": "2026-02-06 15:00:00"
}
```

> top_keywords 来源于 product_cluster_keywords，按 score（或 count）排序取 Top N。
---
...
1. **异步执行（已实现）**: 使用 `/api/products/cluster-large/async` 触发后台任务
2. **进度跟踪（已实现）**: 通过 `/api/tasks/{task_id}` 返回 `progress/message`
